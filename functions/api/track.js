...
import { ensureSchema, json, normalisePhone, orderWithLines, choiceText } from '../_lib/db.js';
...
export async function onRequestGet(context) {
  ...
  return json({
    ...
    // P141 — a pack and the colours asked for
    packs: (o.packs || []).map(p => ({ slug: p.slug, name: p.name, size: p.size, pieces: p.pieces, choice: choiceText(p.choice), price: p.price_paisa / 100, qty: p.qty })),
  });
}
