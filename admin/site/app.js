...
const pages = {
  ...
  async cash(q) {
    ...
    const shiftCols = [{ k: 'business_date', t: 'Day', f: dd }, { k: 'by', t: 'Opened by' }, { k: 'opened_at', t: 'Opened', f: dt }, { k: 'closed_at', t: 'Closed', f: v => v ? dt(v) : '<span class="pill b">open</span>' }, { k: 'opening_float', t: 'Float', num: true, f: rs }, { k: 'expected_cash', t: 'Expected', num: true, f: v => v == null ? '' : rs(v) }, { k: 'declared_cash', t: 'Counted', num: true, f: v => v == null ? '' : rs(v) }, { k: 'variance', t: 'Variance', num: true, f: v => v == null ? '' : `<span class="${cls(Number(v))}">${rs(v)}</span>` }];
    const closeCols = [{ k: 'business_date', t: 'Day', f: (v, r) => `${dd(v)}${r.days > 1 ? `<div class="xs mut">${r.days} days in one close</div>` : ''}${r.closed_with ? `<div class="xs mut">closed with ${dd(r.closed_with)}</div>` : ''}` }, { k: 'by', t: 'By' }, { k: 'locked_at', t: 'At', f: dt }, { k: 'expected', t: 'Expected', num: true, f: v => v == null ? '' : rs(v) }, { k: 'declared', t: 'Counted', num: true, f: v => v == null ? '' : rs(v) }, { k: 'variance', t: 'Variance', num: true, f: v => v == null ? '' : `<span class="${cls(Number(v))}">${rs(v)}</span>` }, { k: 'reopened_at', t: '', f: v => v ? pill('reopened', 'a') : '' }];
