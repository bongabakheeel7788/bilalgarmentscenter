...
export const onRequestGet = guard('cash.move', async (user, db, context) => {
  ...
  const closes = closeRows.map(x => { const d = JSON.parse(x.d); const s = d.summary || {}; return { business_date: d.business_date, locked_at: d.locked_at, by: x.by, reopened_at: d.reopened_at, expected: s.expected, declared: s.declared, variance: s.variance, float_left: s.float_left, closed_with: s.closed_with, days: s.days ? s.days.length : undefined }; });
